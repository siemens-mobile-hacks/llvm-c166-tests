...
#define DEFINE_LAYOUT_FUNCTIONS(fn_prefix, attr0, attr1, attr2, attr3, attr4,  \
                                attr5, attr6)                                  \
  attr0 abi_u16 fn_prefix##_layout_chars0(struct chars3 value, abi_u16 tail) { \
    return (abi_u16)(value.first + value.second * 3U + value.third * 5U +      \
                     tail * 7U);                                               \
  }                                                                            \
  attr1 abi_u16 fn_prefix##_layout_padded1(abi_u16 head, struct padded value,  \
  ...
  attr3 abi_u16 fn_prefix##_layout_union3(abi_u16 head0, abi_u16 head1,        \
  ...
  attr4 abi_u16 fn_prefix##_layout_packed0(ABI_PACKED_REF(packed_value) value, \
                                           abi_u16 tail) {                     \
    return (abi_u16)(value.head * 3U + value.middle * 5U + value.tail * 7U +   \
                     tail * 11U);                                              \
  }                                                                            \
  attr5 abi_u16 fn_prefix##_layout_bits3(                                      \
      abi_u16 head0, abi_u16 head1, abi_u16 head2, struct bit_fields value,    \
      abi_u16 tail) {                                                          \
    return (abi_u16)(head0 * 3U + head1 * 5U + head2 * 7U +                    \
...
#define DEFINE_LAYOUT_BIT_EDGES_FUNCTION(fn_prefix, attr)                      \
  attr abi_u16 fn_prefix##_layout_bit_edges2(                                  \
    ...
    return (abi_u16)(head0 * 3U + head1 * 5U + zero.first * 7U +               \
                     zero.second * 11U + (abi_u16)signed_value.first * 13U +   \
...
#define DEFINE_LAYOUT_NESTED_PACKED_FUNCTION(fn_prefix, attr)                  \
  attr abi_u16 fn_prefix##_layout_nested_packed1(                              \
      ...
      abi_u16 tail) {                                                          \
    return (abi_u16)(head * 3U + nested.prefix * 5U + nested.inner.byte * 7U + \
                     nested.inner.word * 11U + nested.suffix * 13U +           \
                     array.prefix * 17U + array.values[0].byte * 19U +         \
                     array.values[0].word * 23U + array.values[1].byte * 29U + \
