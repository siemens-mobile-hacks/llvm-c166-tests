...
test_u32 TEST_FUNCTION test_bit_writeback(test_u16 value,
                                          test_u16 bit_index,
...
static test_u32 run_form(test_u16 form) {
  ...
  for (bit_index = 0; bit_index != 16U; ++bit_index) {
    ...
    for (index = 0; index != 8U; ++index) {
      ...
      test_u16 old = (value & mask) != 0U;
      test_u16 result = (form & 1U) ? value | mask : value & ~mask;
      test_u16 flags = old ? 1U : 8U;
      ...
      for (initial = 0; initial != 32U; ++initial) {
        ...
        test_u32 actual =
            test_bit_writeback(value, bit_index, initial, form);
...
void main(void) {
  ...
  for (form = 0; form != 12U; ++form)
    tap_is_u32(run_form(form), 0UL, "bit writeback form");
}
