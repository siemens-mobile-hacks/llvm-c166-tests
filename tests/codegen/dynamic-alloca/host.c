...
volatile test_u16 simulator_result;

extern test_u16 llvm_entry_proxy(test_u16 count, test_u16 seed,
                                 test_u16 mode);
...
void main(void) {
  ...
  for (repetition = 0; repetition != 4U; ++repetition) {
    for (index = 0; index != sizeof(counts) / sizeof(counts[0]); ++index) {
      ...
      test_u16 expected = expected_result(counts[index], seed, mode);
      test_u16 actual = llvm_entry_proxy(counts[index], seed, mode);
      if (actual != expected) {
        simulator_result = (test_u16)(0xe000U | (repetition << 8) | index);
        simulator_stop();
      }
    }
  }

  simulator_result = 0xa166U;
  simulator_stop();
}
