...
extern test_u16 alloca_callback(const test_u8 *, test_u16, test_u16,
                                test_u16, test_u16);

C166_NOINLINE C166_USED test_u16 dynamic_alloca(test_u16 count, test_u16 seed,
                                                test_u16 mode) {
  ...
  result = alloca_callback(outer, outer_count, seed, fixed[0], fixed[1]);

  {
    ...
    if (alloca_callback(scratch, scratch_count, scratch_seed, fixed[2],
                        fixed[0]) !=
...

  {
    ...
    result ^= alloca_callback(inner, inner_count, inner_seed, fixed[1],
                              fixed[2]);
  }
