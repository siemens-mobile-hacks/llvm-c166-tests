#include "c166_test.h"
#include "../../abi/abi-mixed-slot-matrix/functions.h"
#include "corpus.inc"
...
typedef abi_u32 (*abi_dispatcher)(
    abi_u16, abi_u16, abi_u16, abi_u16, abi_u16, abi_s8, abi_u8, abi_u32,
    volatile abi_u16 *, abi_callback, abi_u16);

static volatile abi_dispatcher indirect_dispatcher = mixed_dispatch;
...
static abi_u32 call_route(
    abi_u16 route, abi_u16 shape, abi_u16 p0, abi_u16 p1, abi_u16 p2,
    abi_u16 p3, abi_s8 signed_byte, abi_u8 unsigned_byte, abi_u32 long_value,
    volatile abi_u16 *address, abi_callback function, abi_u16 tail) {
  if (route == 0U)
    return call_direct(shape, p0, p1, p2, p3, signed_byte, unsigned_byte,
                       long_value, address, function, tail);
  if (route == 1U)
    return mixed_dispatch(shape, p0, p1, p2, p3, signed_byte, unsigned_byte,
                          long_value, address, function, tail);
  return indirect_dispatcher(shape, p0, p1, p2, p3, signed_byte,
                             unsigned_byte, long_value, address, function,
                             tail);
}
...
static void run_seed(abi_u16 seed_index, abi_u32 seed) {
  ...
  for (iteration = 0U; iteration < ABI_SEEDED_CALL_ITERATIONS; ++iteration) {
    abi_u32 raw0 = next_u32(&state);
    abi_u32 raw1 = next_u32(&state);
    abi_u32 long_value = next_u32(&state);
    abi_u32 misc = next_u32(&state);
    abi_u16 p0 = (abi_u16)raw0;
    abi_u16 p1 = (abi_u16)(raw0 >> 16);
    abi_u16 p2 = (abi_u16)raw1;
    abi_u16 p3 = (abi_u16)(raw1 >> 16);
    abi_s8 signed_byte = (abi_s8)long_value;
    abi_u8 unsigned_byte = (abi_u8)(long_value >> 8);
    volatile abi_u16 cell = (abi_u16)misc;
    abi_u16 tail = (abi_u16)(misc >> 16);
    ...
    if ((iteration & 7U) == 0U) {
      ...
      p2 = 0x7fffU;
      p3 = 0x8000U;
      signed_byte = (abi_s8)0x7fU;
      unsigned_byte = 0U;
      long_value = 0xffffffffUL;
      cell = 0xffffU;
      tail = 0xffffU;
    }
    ...
    for (shape = 0U; shape < ABI_SEEDED_CALL_SIGNATURE_COUNT; ++shape) {
      for (route = 0U; route < ABI_SEEDED_CALL_ROUTE_COUNT; ++route) {
        ...
        abi_u32 actual = call_route(
            route, shape, p0, p1, p2, p3, signed_byte, unsigned_byte,
            long_value, &cell, callback, tail);
        ...
        tap_is_u32(signatures[shape][route],
                   abi_seeded_call_expected[expected_index],
                   "seeded ABI call checkpoint");
      }
    }
    if ((iteration & (ABI_SEEDED_CALL_CHECKPOINT_INTERVAL - 1U)) ==
        ABI_SEEDED_CALL_CHECKPOINT_INTERVAL - 1U)
...
void main(void) {
  ...
  tap_plan(ABI_SEEDED_CALL_CHECKPOINT_COUNT);
