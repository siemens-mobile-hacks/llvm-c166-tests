...
volatile struct abi_seeded_far_arena C166_FAR seeded_far_arena
    C166_SECTION(".seeded_far_arena");

abi_u32 seeded_far_eval(abi_u16 operation,
                        volatile struct abi_seeded_far_arena C166_FAR *arena,
...
static void initialize_arena(abi_u16 seed) {
  ...
  for (index = 0U; index < ABI_SEEDED_FAR_RECORD_COUNT; ++index) {
    volatile struct abi_seeded_far_record C166_FAR *record =
  ...
  for (iteration = 0U; iteration < ABI_SEEDED_FAR_ITERATIONS; ++iteration) {
    ...
    for (operation = 0U; operation < ABI_SEEDED_FAR_OPERATION_COUNT;
      ...
      signatures[operation] = signature_step(
          signatures[operation], (abi_u16)(iteration + 1U),
          seeded_far_eval(operation, &seeded_far_arena, index, other, value));
      if ((iteration & (ABI_SEEDED_FAR_CHECKPOINT_INTERVAL - 1U)) ==
        ...
        abi_u16 expected_index = (abi_u16)(
            ...
            checkpoint);
        tap_is_u32(signatures[operation],
                   abi_seeded_far_expected[expected_index],
                   "far pointer checkpoint");
      }
    }
    if ((iteration & (ABI_SEEDED_FAR_CHECKPOINT_INTERVAL - 1U)) ==
        ABI_SEEDED_FAR_CHECKPOINT_INTERVAL - 1U)
...
void main(void) {
  ...
  tap_plan(ABI_SEEDED_FAR_CHECKPOINT_COUNT);
