...
cat build/reports/tiny/code-size.md
cat build/reports/huge/code-size.md
```
...
|---|---|---:|---|---|
| ABI | `abi-aggregate-args` | L/M/S/T/H | all | ext |
| ABI | `abi-aggregate-layout` | L | all | ext |
| ABI | `abi-aggregate-return` | L/M/S/T/H | all | ext; direct/indirect aggregate returns, mixed live values across callbacks, canaries, register/stack preservation |
| ABI | `abi-byte-return` | L/M | all | ext |
| ABI | `abi-code-bank` | L/S | all | ext |
| ABI | `abi-combined-matrix` | L/M/S/H | O0,O2,Oz | ext |
| ABI | `abi-far-automatic-matrix` | L | all | ext |
| ABI | `abi-far-memory-matrix` | L | all | ext |
| ABI | `abi-far-pointer` | L/M/S/H | all | ext |
| ABI | `abi-generated-aggregate` | L/M/S/H | all | ext |
...
| ABI | `abi-nested-call-chains` | L/M/S/H | all | ext |
...
| ABI | `abi-stack-stop` | L/M/S/H | all | ext |
...
| ABI | `reverse` | L/M/S/H | O2 | ext |
| ABI | `saved` | L/S | O2 | ext |
| ABI | `stack` | L/M/S/T/H | O2 | ext |
| ABI | `structret` | L/M/S/T/H | O2 | ext |
| Codegen | `aggregate` | L/M/S/H | O2 | ext |
| Codegen | `alu-direct-memory` | L/M/S/H | O2 | ext/ext2; 290816 ALU/compare-and-loop result/flags cases, GPR/SFR register/memory/immediates, word/byte indirect addressing and postincrement |
| Codegen | `c-assembly` | L/M/S/H | all | ext; shared C and preprocessed assembly, C↔assembly calls, macros, local labels, data/BSS |
| Codegen | `backend-tuple-overlap` | L/S | O2 | ext |
| Codegen | `branch` | L/M/S/H | O2 | ext |
| Codegen | `code-segment-boundaries` | L/M/S/H | O2 | ext/ext2; JMPR/JMPA/JB IP-wrap and code-segment fetch boundaries |
| Codegen | `computed-goto` | L/M | all | ext |
| Codegen | `conditional-transfers` | L/M/S/H | O2 | ext/ext2; 3072 condition/flags/stack cases |
| Codegen | `dynamic-alloca` | L/M/S/T/H | all | ext; builtin/aligned alloca, nested VLA lifetimes, fixed locals, callback stack arguments, early exits and repeated-call stack balance |
| Codegen | `extended-addressing` | L/M/S/H | O2 | ext/ext2; 128 EXT read/store cases plus 64 nested EXTP/EXTS cases, counts 1..4, counter reload and expiry |
| Codegen | `esfr-addressing` | L/M/S/H | O2 | ext/ext2; 68 SFR/ESFR routing/expiry cases plus 1280 EXT/ATOMIC flags cases; bit operations, MOV writes, PUSH reads, stack balance, counts 1..4 |
| Codegen | `external-branch-relaxation` | L/S | O2 | ext |
| Codegen | `jump-table` | L/M/S/H | O1,O2,O3,Os,Oz | ext |
| Codegen | `near-call-stack` | L/M/S/H | O2 | ext/ext2; CALLR/PCALL/RETP stack and flags |
| Codegen | `mov-memory` | L/M/S/H | O2 | ext/ext2; 5760 SFR/direct/indirect transfer/flags cases, byte predecrement, immediate and neighbor preservation |
| Codegen | `mov-indirect` | L/M/S/H | O2 | ext/ext2; 248064 word/byte load/store/copy cases, predecrement/postincrement, displacement, RL/RH registers, even/odd byte addresses, flags and source/neighbor preservation |
| Codegen | `sfr-bitfields` | L/M/S/H | all | ext; SFR/ESFR word access, bit fields, and attributed bit declarations |
| Codegen | `mov-extend` | L/M/S/H | O2 | ext/ext2; 81920 MOVBS/MOVBZ result/flags cases, all byte values, GPR/SFR/memory and source preservation |
| Codegen | `bit-writeback` | L/M/S/H | O2 | ext/ext2; 49152 JBC/JNBS cases, GPR/RAM boundaries, flags, short/long branches and neighbor preservation |
| Codegen | `bit-branch` | L/M/S/H | O2 | ext/ext2; 49152 JB/JNB cases, taken/not-taken, operand/flags preservation, GPR/RAM, short/long branches; shared sources with bit-writeback |
| Codegen | `bit-binary` | L/M/S/H | O2 | ext/ext2; 61440 BMOV/BMOVN/BAND/BOR/BXOR/BCMP result/flags cases, GPR/RAM aliasing, source and neighbor preservation |
| Codegen | `bit-update` | L/M/S/H | O2 | ext/ext2; 24576 BSET/BCLR result/flags cases, all bit indices, GPR/RAM boundaries and neighbor preservation |
| Codegen | `bit-field` | L/M/S/H | O2 | ext/ext2; 49152 BFLDL/BFLDH mask/data/result/flags cases, GPR/RAM boundaries, other-byte and neighbor preservation |
| Codegen | `trap` | L/M/S/H | O2 | ext/ext2; 1536 segmented TRAP/RETI cases, three vectors, flags/priority, stack frame and CSP/SP restoration |
| Codegen | `protocol` | L/M/S/H | all | ext |
| Codegen | `shift-rotate` | L/M/S/H | O2 | ext/ext2; 103680 SHL/SHR/ASHR/ROL/ROR result/flags cases, count masking, sign-fill and destination/count aliasing |
| Differential | `control-flow-matrix` | L/M/S/T/H | all | ext |
| Differential | `far-pointer-alias-matrix` | L/M/S/H | all | ext |
| Differential | `float-varargs-matrix` | L/M/S/H | all | ext; bidirectional C variadic calls, float promotion, five fixed-argument shapes, exact FP words |
| Differential | `float-volatile-matrix` | L/S | all | ext |
| Differential | `float-width-matrix` | L/M/S/T/H | all | ext |
| Differential | `float32-arithmetic-matrix` | L | all | ext |
| Differential | `float32-conversion-matrix` | L/M/S/T/H | all | ext |
| Differential | `float32-storage-matrix` | L | all | ext |
| Differential | `float64-arithmetic-matrix` | L/M/S/T/H | all | ext |
| Differential | `float64-conversion-matrix` | L/M/S/T/H | all | ext |
| Differential | `float64-storage-matrix` | L | all | ext |
| Differential | `float64-snapshot` | L/M/S/T/H | all | ext |
| Differential | `float64-tasking-reverse` | L | all | ext |
| Differential | `generated-type-semantics` | L/M/S/T/H | all | ext |
| Differential | `integer16-matrix` | L/M/S/T/H | all | ext |
| Differential | `integer32-matrix` | L/M/S/T/H | all | ext |
| Differential | `memory-builtins` | L/M/S/H | all | ext |
| Differential | `metamorphic-conversion-matrix` | L/M/S/T/H | all | ext |
| Differential | `pseudo-expansion-matrix` | L/M/S/H | all | ext |
| Differential | `scalar-float-bytes` | L/M/S/T/H | all | ext |
...
| Stress | `seeded-f32-differential` | L/M/S/H | all | ext; `100k` |
| Stress | `seeded-f32-to-i32` | L/M/S/H | all | ext; `100k` |
...
| Stress | `seeded-f64-to-f32` | L/M/S/H | all | ext; `100k` |
| Stress | `seeded-f64-sub` | M | all | ext |
| Stress | `seeded-far-pointer` | L | all | ext; `10k` |
| Stress | `seeded-integer-differential` | L/M/S/H | all | ext; Large `100k` |
| Size | `code-size-corpus` | L/M/S/T/H | LLVM Os / TASKING O2 | generated source-object size; external runtimes excluded |
| Size | `code-size-floating` | L/M/S/T/H | Os | shared float32/binary64 arithmetic, conversion and comparison corpus with linked runtime |
